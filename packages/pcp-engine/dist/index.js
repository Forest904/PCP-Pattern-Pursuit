...
const buildAlphabet = (size) => ALPHABET_POOL.slice(0, clampInt(size, 1, ALPHABET_POOL.length));
const resolveSettings = (preset, rng, overrides) => {
    const base = PRESETS[preset];
    const tileCountRange = overrides?.tileCount
    ...
    const tileCount = overrides?.tileCount ?? clampInt(tileCountRange[0] + Math.floor(rng() * tileCountSpan), 2, 24);
    ...
    const theme = overrides?.theme ?? "preset";
...
export const generatePuzzle = ({ preset, seed, overrides }) => {
